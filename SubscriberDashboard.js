...
// --- Minimal Icon Components (Local for use in this component) ---
const SearchIcon = () => (
...
function SubscriberDashboard() {
    ...
    // The state and toggle function are retrieved from the Layout context
    ...
    // showNotifications state is now derived from context, not local useState
    ...
    const handleNavigateClick = (path) => {
        console.log(`Navigating to Reports page for group: ${path}`);
        navigate('/reports'); 
    };
    ...
    // State for controlling the main table view: 'subscribed', 'unsubscribed', or 'pending'
    const [activeTable, setActiveTable] = useState('subscribed'); 
    
    // Handle subscription request (remains the same)
    const handleSubscriptionRequest = (group) => {
        ...
        setActiveTable('pending'); // Auto switch to pending tab
    ...
    // Admin Approval Placeholder (remains the same)
